...
type Queue interface {
  Enqueue(customer Customer) (cus Customer)
  Dequeue() (Customer)
  Len() (int)
...
func Run(arrivalDistribution Distribution, q Queue, serviceDistribution Distribution) (rejects, completes <-chan Customer) {
  rejected := make(chan Customer)
  completed := make(chan Customer)
  var clock float64 // global time
    ...
    for clock < 10 {
      if (t1 < t2) {                                      // If next arrival is before next completion -> Event: Arrival
        ...
        if (!math.IsInf(t2, 1)) {
          ...
          customer.Completion = t2
          customer.Start = t2 - customer.Service
          completed <- customer                    // add the customer to the completed channel
        }
        t2 = q.NextCompletion();
      }
    }
